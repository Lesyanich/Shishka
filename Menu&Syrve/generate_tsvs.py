...
categories_data = [
    ...
    ("CAT-FERMENTED_SF",  "Fermented & Pickled (SF)",  "GRP-SF"),     # v4
    ...
    ("CAT-SIDES_SALE",    "Functional Sides",          "GRP-SALE"),   # v4
]
...
items = [
    ...
    # ── Kraut raw inputs (v4) ─────────────────────────────────────────────────
    ("White Cabbage",         "good",           "Product",  "",   "",        "RAW",  "Yes"),
    ("Fine Salt",             "good",           "Product",  "",   "",        "RAW",  "Yes"),
    # ── Fermented SF (v4) ────────────────────────────────────────────────────
    ("SF Fermented Cabbage",  "dish",           "Product",  "3.2","kg",      "PF",   "Yes"),
    # ── Sale Dishes ───────────────────────────────────────────────────────────
    ("Borsch Bio-Active (portion)", "dish",     "Compound", "1",  "portion", "SALE", "Yes"),
    ("Kraut Side (portion)",        "dish",     "Compound", "1",  "portion", "SALE", "Yes"),  # v4
    # ── Internal Accounting ───────────────────────────────────────────────────
...
ext = {
    ...
    "Borsch Bio-Active (portion)": ("portion", "GRP-SALE", "CAT-SOUPS_SALE",
                                    "35", "1.2", "0.5", "6.5"),
    # ── Kraut inputs (v4) ────────────────────────────────────────────────────
    "White Cabbage":     ("kg", "GRP-INGREDIENTS", "CAT-VEGETABLES",
                          "25",  "1.3", "0.1",  "5.8"),
    "Fine Salt":         ("kg", "GRP-INGREDIENTS", "CAT-SPICES",
                          "",   "",    "",     ""),
    # ── Fermented SF (v4, per 100g, ESTIMATE) ────────────────────────────────
    "SF Fermented Cabbage": ("kg", "GRP-SF", "CAT-FERMENTED_SF",
                              "19", "0.9", "0.1", "4.3"),
    # ── Kraut Sale (v4, ESTIMATE) ─────────────────────────────────────────────
    "Kraut Side (portion)": ("portion", "GRP-SALE", "CAT-SIDES_SALE",
                              "19", "0.9", "0.1", "4.3"),
    # ── Internal ─────────────────────────────────────────────────────────────
    "Processing Loss":   ("",  "",  "", "", "", "", ""),
}
...
yield_dict = {
    ...
    # v4 — Kraut inputs
    # Cabbage: TTK qty already pre-yield (outer leaves used as cover, not waste)
    "White Cabbage":       1.00,
    "Fine Salt":           1.00,
    "SF Fermented Cabbage": 1.00,
}
...
with open('BOM.tsv', 'w', newline='') as f:
    ...
    # ── v4: SF Fermented Cabbage — output 3.2 kg batch ───────────────────────
    # Input: 3.6kg cabbage (yield=1.00) + 0.4kg carrot (yield=0.80) + 72g salt
    # Net = 3.6 + 0.32 + 0.072 = 3.992 kg
    # Output 3.2 kg kraut; loss = 0.792 kg (fermentation brine — saved for dressings per ZW policy)
    write_balanced_bom(w, "SF Fermented Cabbage", [
        ("White Cabbage", "3.600", "kg"),
        ("Fresh Carrot",  "0.400", "kg"),
        ("Fine Salt",     "0.072", "kg"),
    ])

    # v4: Kraut sale portion — 1 portion = 80g SF Fermented Cabbage
    p = "Kraut Side (portion)"
    w.writerow([nom[p]["short_code"], p,
                nom["SF Fermented Cabbage"]["short_code"],
                "SF Fermented Cabbage", "0.08", "kg"])
...
with open('Production_Flow.tsv', 'w', newline='') as f:
    ...
    # ── v4: SF Fermented Cabbage (Probiotic Kraut) ───────────────────────────
    # SOP: Probiotic Kraut L1 | Batch 4kg in → 3.2kg out | 3-5 days @ 20-22°C
    p = "SF Fermented Cabbage"
    w.writerow([nom[p]["short_code"], p, "Slicing",      "MANUAL",       "",    "15","No",
                "Cabbage 2mm strips; carrot thin straws; reserve 2 outer cabbage leaves for cover"])
    w.writerow([nom[p]["short_code"], p, "Massaging",    "MANUAL",       "",     "5","No",
                "Mix veg + salt; massage GENTLY 3-5 min until wet. Do NOT crush!"])
    w.writerow([nom[p]["short_code"], p, "Packing",      "MANUAL",       "",     "5","No",
                "Layer into 5L press container; press with fist; cover with reserved outer leaves"])
    w.writerow([nom[p]["short_code"], p, "Fermenting",   "FERM-01",   "21", "5760","Yes",
                "20-22°C; 3-5 days (avg 4 days); poke to bottom daily to release gas; pH target 3.8-4.2"])
    w.writerow([nom[p]["short_code"], p, "Quality Check","MANUAL",       "",     "5","No",
                "Texture=Crunchy (no mushy); pH 3.8-4.2; discard & restart if soft or wrong smell"])
...
uom_rows = [
    ...
    # ── Kraut raw inputs (v4) ──────────────────────────────────────────────────
    ("White Cabbage",       "kg",     "Bag 10kg",       "10",  "kg",      "1"),
    ("Fine Salt",           "kg",     "Pack 1kg",        "1",  "kg",      "1"),
    # ── Fermented SF (v4); Sale_Ratio = portion size 80g in kg ────────────────
    ("SF Fermented Cabbage","kg",     "Container 3.2kg", "3.2","portion", "0.08"),
    # ── Sale Dishes ────────────────────────────────────────────────────────────
    ("Borsch Bio-Active (portion)", "portion", "Bowl", "1", "portion", "1"),
    ("Kraut Side (portion)",        "portion", "Bowl", "1", "portion", "1"),   # v4
]
